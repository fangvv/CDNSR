...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0)
        if resume_state is None:
            ...
            util.mkdirs((path for key, path in opt['path'].items() if not key == 'experiments_root' and not key == 'pretrain_proj_head'
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            ### training
            if hasattr(model.netG.module, "set_tau"):
                tau = max(1 - (current_step - 1) / (total_iters), 0.4)
                # print("tau: ", tau, "current_step: ", current_step, ", total_iters: ", total_iters)
                model.netG.module.set_tau(tau)
            else:
                raise NotImplementedError('set_tau is not recognized.')
