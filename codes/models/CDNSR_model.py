...
from utils.util import Get_FLOPs
from models.loss import CharbonnierLoss,class_loss_3class,average_loss_3class,EE_flops_loss,ContrastLoss,PerceptualLoss,KDL1Loss,CSDLoss
...
class CDNSR_Model(BaseModel):
    def __init__(self, opt):
        super(CDNSR_Model, self).__init__(opt)
        ...
        if self.is_train:
            ...
            self.flops_loss_w = float(opt["train"]["flops_loss_w"])
            self.cs_loss_w = float(opt["train"]["cs_loss_w"])
            self.csd_loss_w = float(opt["train"]["csd_loss_w"])
            self.neg_num = int(opt["train"]["neg_num"])
            self.pf = opt['logger']['print_freq']
            self.batch_size = int(opt['datasets']['train']['batch_size'])
            self.perceptual_loss_w = float(opt["train"]["perceptual_loss_w"])
            self.kd_l1_loss_w = float(opt["train"]["kd_l1_loss_w"])
            self.netG.train()
            ...
            if loss_type == 'l1':
                ...
                self.flops_loss = EE_flops_loss(Get_FLOPs(opt['network_G']['which_model_G']), opt["train"]["target_flops"]).to(self.device)
                self.cs_loss = ContrastLoss(weights=[0, 0, 0, 0, 1.0],d_func="cos",t_detach=True,save_path=self.opt['path']['models'], loadpath=self.opt['path']['pretrain_proj_head']).to(self.device)
                self.csd_loss = CSDLoss(weights=[1.0/32, 1.0/16, 1.0/8, 1.0/4, 1.0],d_func="L1",t_detach=True).to(self.device)
                self.upsampler = nn.Upsample(scale_factor=4, mode='bicubic')
                self.perceptual_loss = PerceptualLoss().to(self.device)
                self.kd_l1_loss = KDL1Loss(self.cri_pix).to(self.device)
            else:
            ...
            if opt['fix_CLASS_module']:
                for k, v in self.netG.named_parameters():  # can optimize for a part of the model
                    if v.requires_grad and "class" in k:
                        v.requires_grad=False

            for k, v in self.cs_loss.named_parameters():  # can optimize for a part of the model
                if v.requires_grad:
                    optim_params.append(v)
                    logger.warning('[SS Module] Params [{:s}] will be optimize.'.format(k))
    ...
    def optimize_parameters(self, step):
        if hasattr(self.netG, "eval_bn"):
            self.netG.eval_bn()
        self.optimizer_G.zero_grad()
        [self.fake_H, self.fake_Hs], self.type = self.netG(self.var_L, self.is_train)
        #print(self.type)
        l_pix = self.cri_pix(self.fake_H, self.real_H)
        class_loss=self.class_loss(self.type)
        average_loss=self.average_loss(self.type)
        flop_loss = self.flops_loss(self.type)

        cs_loss = torch.tensor(0).cuda()
        csd_loss = torch.tensor(0).cuda()
        perceptual_loss = torch.tensor(0).cuda()
        kd_l1_loss = torch.tensor(0).cuda()

        if self.perceptual_loss_w != 0:
            perceptual_loss=self.perceptual_loss(self.fake_Hs, self.type)
        
        if self.neg_num != 0 and self.cs_loss_w != 0:
            bic_sample = self.var_L[torch.randperm(self.neg_num), :, :, :]
            bic_sample = self.upsampler(bic_sample)
            cs_loss = self.cs_loss(self.fake_Hs[-1], self.fake_Hs[0], bic_sample)
            cs_loss += self.cs_loss(self.fake_Hs[-1], self.fake_Hs[1], bic_sample)

        if self.csd_loss_w != 0:
            bic_sample = self.var_L[torch.randperm(self.neg_num), :, :, :]
            bic_sample = self.upsampler(bic_sample)
            csd_loss = self.csd_loss(self.fake_Hs[-1], self.fake_Hs[0], bic_sample)
            csd_loss += self.csd_loss(self.fake_Hs[-1], self.fake_Hs[1], bic_sample)

        if self.kd_l1_loss_w != 0:
            kd_l1_loss = self.kd_l1_loss(self.real_H ,self.fake_Hs, self.type)

        loss =  self.l1w * l_pix + \
                self.class_loss_w * class_loss + \
                self.average_loss_w * average_loss + \
                self.flops_loss_w * flop_loss + \
                self.cs_loss_w * cs_loss + \
                self.perceptual_loss_w * perceptual_loss + \
                self.kd_l1_loss_w * kd_l1_loss
        ...
        self.log_dict['flop_loss'] = flop_loss.item()
        self.log_dict['constractive_loss'] = cs_loss.item()
        self.log_dict['perceptual_loss'] = perceptual_loss.item()
        self.log_dict['kd_l1_loss'] = kd_l1_loss.item()
    ...
    def test(self):
        ...
        # psnr_log_path = "/home/wq/workplace/classsr/script/our_fsrcnn.log"
        # f1 = open(psnr_log_path,  'a+')
        ...
        for idx, (LR_img,GT_img) in enumerate(zip(lr_list,gt_list)):
            ...
            # f1.writelines("{}\n".format(psnr))
            # if idx == 147 or idx == 103:
            #     sr_img[0:4,:,1] = 255
            #     sr_img[395:399,:,1] = 255
            #     sr_img[:,0:4,1] = 255
            #     sr_img[:,395:399,1] = 255
            #     self.fake_H = sr_img
            #     print(sr_img.shape)
    ...
    def save(self, iter_label):
        ...
        if self.cs_loss is not None:
            self.cs_loss.save_proj_head(iter_label)
