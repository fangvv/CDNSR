...
class EE_CARN_M(nn.Module):
    def __init__(self, in_nc, out_nc, nf, scale=4, multi_scale=False, group=4):
        super(EE_CARN_M, self).__init__()
        self.scale = scale
        rgb_range = 1
        rgb_mean = (0.4488, 0.4371, 0.4040)
        rgb_std = (1.0, 1.0, 1.0)
        ...
        self.upsample1 = arch_util.UpsampleBlock(nf, scale=scale, 
                                          multi_scale=multi_scale,
                                          group=group)
        self.upsample2 = arch_util.UpsampleBlock(nf, scale=scale, 
                                          multi_scale=multi_scale,
                                          group=group)
        self.upsample3 = arch_util.UpsampleBlock(nf, scale=scale, 
                                          multi_scale=multi_scale,
                                          group=group)
        self.exit1 = nn.Conv2d(nf, out_nc, 3, 1, 1)
        self.exit2 = nn.Conv2d(nf, out_nc, 3, 1, 1)
        self.exit3 = nn.Conv2d(nf, out_nc, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        out1 = self.upsample1(o1, scale=self.scale)
        out1 = self.exit1(out1)
        out1 = self.add_mean(out1)

        b2 = self.b2(o1)
        c2 = torch.cat([c1, b2], dim=1)
        o2 = self.c2(c2)
        
        out2 = self.upsample2(o2, scale=self.scale)
        out2 = self.exit2(out2)
        ...
        out3 = self.upsample3(o3, scale=self.scale)

        out3 = self.exit3(out3)
        out3 = self.add_mean(out3)

        outs = [out1, out2, out3] # [336 , 457, 582]M MACS
