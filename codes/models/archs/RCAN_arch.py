...
## Residual Channel Attention Block (RCAB)
class CSRCAB(nn.Module):
    def __init__(
        self, conv, n_feat, kernel_size, reduction,
        bias=True, bn=False, act=nn.ReLU(True), res_scale=1):
        assert(bn == False)

        super(CSRCAB, self).__init__()
        modules_body = []
        ...
        self.CA_body = nn.Sequential(*[CALayer(n_feat, reduction)])
        self.res_scale = res_scale
    ...
    def forward(self, x, ch_mask=None):
        ...
        if ch_mask == None:
            ...
            for i in range(len(self.body)):
                if isinstance(self.body[i], nn.Conv2d):
                    res_input = res
                    res = self.body[i](res_input)
                    res = res * ch_mask[:, :, :, 1:] + res_input * ch_mask[:, :, :, :1]
                elif isinstance(self.body[i], nn.ReLU):
                    res = self.body[i](res)
                else:
                    raise RuntimeError("must be conv2d or relu!!")
        res = self.CA_body(res)
        # res = self.body(x)
        #res = self.body(x).mul(self.res_scale)
        res += x
        return res
