...
class BaseModel():
    ...
    def load_network(self, load_path, network, strict=True):
        ...
        try:
            ...
            print("different classifier, it is ok")
    ...
    def load_network_ee_classSR_3class(self,load_path, network, strict=True):
        if isinstance(network, nn.DataParallel) or isinstance(network, DistributedDataParallel):
            network = network.module.net
        load_net = torch.load(load_path)
        load_net_clean = OrderedDict()  # remove unnecessary 'module.'
        for k, v in load_net.items():
            if k.startswith('module.'):
    ...
    def resume_training(self, resume_state):
        ...
        for i, o in enumerate(resume_optimizers):
            try:
                ...
                print("ValueError: loaded state dict contains a parameter group that doesn't match the size of optimizer's group")
